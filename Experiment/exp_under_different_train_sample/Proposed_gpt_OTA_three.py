...
from MI_calc import calculate_scores, filter_three_rows
...
# seed = 1:W8,W7,W5,W6,L2,L1,c2,W2,W3,L4,L7,L6,L3,L5,W4,c1,W1,L8
# seed = 2:W8,W7,W2,L1,W1,L6,W5,W6,L4,L8,c2,c1,L3,W3,L2,L7,W4,L5
# seed = 3:W8,W7,L7,L8,W3,L2,L4,W2,c2,W4,L3,c1,W5,W6,L6,L5,L1,W1
# seed = 4:L8,W8,L7,W7,W3,W4,c2,L1,W1,L5,W5,L3,c1,L2,W2,L6,L4,W6()
# seed = 5:
# seed = 6:
# seed = 7:W7,L8,W6,L7,W4,W8,L5,L3,W5,L6,W1,c1,W3,L4,W2,L2,L1,c2
...
def sort_and_group(scores):
    ...
    param_scores = zip(["c1", "c2", "L1", "L2", "L3", "L4", "L5", "L6", "L7", "L8", "W1", "W2", "W3", "W4", "W5", "W6", "W7", "W8"], scores)
    # 根据分数从高到低进行排序
    sorted_params = sorted(param_scores, key=lambda x: x[1], reverse=True)
    # 分组，每四个为一组，最后两个为一组
    grouped_params = [sorted_params[i:i+4] for i in range(0, 16, 4)] + [sorted_params[16:]]
...
def cal_weight(y, best_i):
    ...
    for i, row in enumerate(y):
        # 检查是否满足特定条件
        if (row[0] > 80 and row[1]  < ((1e-3 / 1.8 + best_i) / 2) and row[2] > 60 and row[3] > 2e6):
            weight += 1
    return weight
...
if __name__ == "__main__":
    ...
    # 使用函数筛选
    FoM_y, FoM_num = filter_three_rows(dby)
    # FoM作为y的最后一列指标
    dby = torch.cat((dby, FoM_y), dim=1)
    # 计算每个参数的SA分数和更新权重
    scores = calculate_scores(dbx=dbx, dby=dby, FoM_num=FoM_num, I_num=I_num, gain_num=gain_num, GBW_num=GBW_num, phase_num=phase_num,
                              iter=iter_1, init_num=init_num, n_neighbors=5, n_repeats=100, input_dim=18)
    ...
    dbx_columns = ["c1", "c2", "L1", "L2", "L3", "L4", "L5", "L6", "L7", "L8", "W1", "W2", "W3", "W4", "W5", "W6", "W7", "W8"]
    ...
    # Save as CSV files
    df_dbx.to_csv(f'dbx_three_{SEED}.csv', index=False)
    df_dby.to_csv(f'dby_three_{SEED}.csv', index=False)
    ...
    # 参数名称列表
    param_names = ['c1', 'c2', 'L1', 'L2', 'L3', 'L4', 'L5', 'L6', 'L7', 'L8', 'W1', 'W2', 'W3', 'W4', 'W5', 'W6', 'W7', 'W8']
    # 定义两个范围
    percentage1 = 0.5
    percentage2 = 0.5
    # 获取解绑参数的索引和范围
    ...
    bo = BayesianOptimization(
        ...
        simulation_function=OTA_three_simulation_all,
        objective=objective,
        posterior_transform=posterior_transform,
        mode='collect_stage',
    ...
    # 运行优化过程
    best_params, best_simulation_result5, last_x, last_y, dbx, dby1, gain_num, I_num, GBW_num, phase_num, last_all_x, all_x = (
        bo.find(stage_init_num=stage_init))
    initial_values = last_all_x[-1]  # 参数更新
    ...
    # 运行优化过程
    best_params, best_simulation_result5, last_x, last_y, dbx, dby2, gain_num, I_num, GBW_num, phase_num, last_all_x, all_x = (
        bo.find(stage_init_num=stage_init))
    initial_values = last_all_x[-1]  # 参数更新
    w1_num = cal_weight(dby1, last_y[1])
    w2_num = cal_weight(dby2, last_y[1])
    ...
    print('第六阶段 param17:', param17, 'param18:', param18)
    ...
    # 运行优化过程
    best_params, best_simulation_result6, last_x, last_y, dbx, dby1, gain_num, I_num, GBW_num, phase_num, last_all_x, all_x = (
        bo.find(stage_init_num=stage_init))
    ...
    # 运行优化过程
    best_params, best_simulation_result6, last_x, last_y, dbx, dby2, gain_num, I_num, GBW_num, phase_num, last_all_x, all_x = (
        bo.find(stage_init_num=stage_init))
    best_simulation_result = best_simulation_result + best_simulation_result6
    print('stage6_result:', best_simulation_result6)
