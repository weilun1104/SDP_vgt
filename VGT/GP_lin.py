...
# GP Model
class GP(ExactGP):
    def __init__(self, train_x, train_y, likelihood, lengthscale_constraint, outputscale_constraint, ard_dims, mean_y, std_y, bounds,lin_model):
        ...
        self.mean_y = mean_y
        self.std_y = std_y
        self.min_y = (train_y+ self.lin_model.predict(train_x)).min()
    ...
    def predict(self, xtensor):
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            mu = (predict.mean+ self.lin_model.predict(xtensor)) * self.std_y + self.mean_y  
    ...
    def LCB_nograd(self, xtensor, kappa = 1.5):
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            x_tensor = norm_x(xtensor,self.bounds)
            predict = self.likelihood(self.__call__(x_tensor))
            mu = predict.mean + self.lin_model.predict(xtensor) 
            std = torch.sqrt(predict.variance)
            lcb = mu - kappa * std
        return lcb
    ...
    def PI_nograd(self, xtensor):
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            z = (self.min_y - mu)/std
            '''
            print('linear = ',self.lin_model.predict(xtensor))
            print('mu = ',mu)
            print('std = ',std)
            '''
            #z = self.min_y - mu
...
import time
from .linear_regression import train_lin_reg
...
def train_gp(train_x, train_y, use_ard, num_steps, bounds=None, hypers={}):
    ...
    #linear model    
    lin_model, lin_loss = train_lin_reg(normed_train_x.clone().detach(),normed_train_y.clone().detach(),100)
    print('lin_loss = ',lin_loss[-1])
    
    GP_train_y = normed_train_y.clone().detach() - lin_model.predict(normed_train_x.clone().detach())
    ...
    if hypers:
        ...
        hypers["likelihood.noise"] = 0.000005
        model.initialize(**hypers)
