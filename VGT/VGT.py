...
class VGT():
    def __init__(self, min_current,best_all_y,best_y, f, lb, ub, n_init, max_iter, Cp=0.1, use_approximation = False, N_neighbor = None, use_subspace = False, SIR_parameter = (4,3), _Rp = 4, num_samples = 20000, N_bars = None):
        # self.valid_x = valid_x
        self.min_current = min_current
        self.best_all_y = best_all_y
        self.best_y = best_y
        self.lb = lb
        self.ub = ub
        self.dim = len(lb)
    ...
    def random_sample(self,n_init):
        ...
        # x_sample = np.concatenate((valid_x, x_sample), axis=0)
        x_new,y,self.min_current,self.best_all_y,self.best_y = self.f(x_sample,self.min_current,self.best_all_y,self.best_y)
        x_new = x_new.to(torch.double)
        x_new = x_new.to('cpu')
        self.opt_idx = np.argmin(y)
        self.y_opt = y[self.opt_idx]
        self.x_opt = x_new[self.opt_idx].cpu().numpy()
        
        self.current_pnts = x_new.size(0)-1   
        
        if self.use_approximation:
            for j in range(x_new.size(0)):
                for i in range(j):
                    self.dist_matrix[j,i] = self.dist_matrix[i, j] = np.abs(x_new[j]-x_new[i]).sum()
        
        self.build_graph()
        
        for j in range(x_new.size(0)):
            x_numpy = x_new.cpu().numpy()
            newnode = node(x_numpy[j,:],y[j],j,x_new.size(0),self.Cp)
    ...
    def collect_sample(self,x):
        ...
        x_new,y,self.min_current,self.best_all_y,self.best_y = self.f(x_expand,self.min_current,self.best_all_y,self.best_y)
        if x_new.numel() != 0:
            x_new = x_new.squeeze(0).cpu().numpy()
            self.current_pnts += 1
            if self.use_approximation:
                for j in range(self.current_pnts):
                    self.dist_matrix[j,self.current_pnts] = self.dist_matrix[self.current_pnts, j] = np.abs(self.nodes[j].x-x_new).sum()
            if self.y_opt > y:
                self.y_opt = y
                self.dk = x_new - self.x_opt
                self.x_opt = x_new
                self.opt_idx = self.current_pnts
            self.build_graph()
            
            
            newnode = node(x_new,y[0],self.current_pnts,self.current_pnts+1,self.Cp)
            self.nodes.append(newnode)
    ...
    def propose_sample_PI_k(self, num_samples):
        ...
        def pi(Xin):
            ...
            outcell = self.kdtree.query(x.cpu().detach().numpy(), eps=0, k=1)[1] != target_idx 
        ...
        for j in range(10):
            ...
            pi_val[incell] = self.GP.PI_nograd(torch.tensor(x_sample[incell])).cpu().detach().numpy()
    ...
    def search(self):
        ...
        n = self.random_sample(self.n_init)
        # for j in range(self.max_iter - n):
        while len(self.best_all_y)<400:
            ...
            print('current length of best_all_y:', len(self.best_all_y))
            #print('current best x:',self.x_opt)
        
        # return (self.x_opt,self.y_opt)
        return self.best_all_y
